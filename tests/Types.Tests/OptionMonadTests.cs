...
public class OptionMonadTests
{
    ...
    public void Bind_CallOnSome_ReturnsNewSomeValue()
    {
        ...
        var newOptionMonad = SomeOptionMonad.Bind(
            value =>
                new Monad<OptionType<DummyNewValue>>(
    ...
    {
        var newOptionMonad = NoneOptionMonad.Bind(
            value =>
                new Monad<OptionType<DummyNewValue>>(
                    new DummyNewValue { NameAllCaps = value.Name.ToUpper() }
                )
    ...
    public async Task BindAsync_CallOnSome_ReturnsNewSomeValue()
    {
        ...
        var newOptionMonad = await optionMonadTask.BindAsync(
            value =>
                Task.FromResult(
                    (IMonad<OptionType<DummyNewValue>>)
                        new Monad<OptionType<DummyNewValue>>(
                            new DummyNewValue { NameAllCaps = value.Name.ToUpper() }
                        )
                )
