...
using Monad = NetFunctional.Types.Traits.Monad;
...
public class ResultConstructTests
{
    ...
    {
        var expected = new Functor<ResultType<DummyValue, DummyError>>(
    ...
    public void FunctorStringErrorConstruct_CallWithNullValue_ReturnErrorResult()
    {
        ...
        var expected = new Functor<ResultType<DummyValue>>(
            new ResultType<DummyValue>.Error(expectedMessage)
        );
        Functor.Result.Construct(NullValue, Error).Should().BeEquivalentTo(expected);
    }
    ...
    {
        var expected = new Monad<ResultType<DummyValue, DummyError>>(
            new ResultType<DummyValue, DummyError>.Ok(Value)
        );
    ...
    {
        var expected = new Monad<ResultType<DummyValue>>(new ResultType<DummyValue>.Ok(Value));
    ...
    {
        var expected = new Result<DummyValue, DummyError>(
            new ResultType<DummyValue, DummyError>.Ok(Value)
        );
    ...
    {
        var expected = new Result<DummyValue>(new ResultType<DummyValue>.Ok(Value));
    ...
    {
        const string expectedMessage = "error message";
        var expected = new Result<DummyValue>(new ResultType<DummyValue>.Error(expectedMessage));
        Result.Construct(NullValue, Error).Should().BeEquivalentTo(expected);
    }
}
