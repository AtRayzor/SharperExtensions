...
public class ResultFunctorTests
{
    ...
    {
        var expected =
            new Functor<ResultType<DummyNewValue, DummyError>>(
    ...
    {
        var expected =
            new Functor<ResultType<DummyValue, DummyNewError>>(new ResultType<DummyValue, DummyNewError>.Ok(Value));
        var resultTask = Task.FromResult(OkResultFunctor);
    ...
    {
        var expected = new Functor<ResultType<DummyNewValue>>(new ResultType<DummyNewValue>.Ok(NewValue));
    ...
    {
        var expected = new Functor<ResultType<DummyNewValue>>(new ResultType<DummyNewValue>.Error(ErrorMessage));
    ...
    {
        var expected = new Functor<ResultType<DummyValue>>(new ResultType<DummyValue>.Error($"new {ErrorMessage}"));
        var resultTask = Task.FromResult(StringErrorResultFunctor);
    ...
    {
        var expected =
            new Functor<ResultType<DummyNewValue, DummyNewerError>>(
    ...
    {
        var expected =
            new Functor<ResultType<DummyNewValue, DummyNewError>>(
    ...
    {
        var expected =
            new Functor<ResultType<DummyNewValue>>(
    ...
    public async Task MatchAsync_CallOnOkResultFunctor_ReturnsNewOkResult()
    {
        var expected =
            new Functor<ResultType<DummyNewValue, DummyNewerError>>(
                new ResultType<DummyNewValue, DummyNewerError>.Ok(NewValue));
        (await Task.FromResult(OkResultFunctor).MatchAsync(OkMap, ErrorMap)).Should().BeEquivalentTo(expected);
    }
    ...
    public async Task MatchAsync_CallOnOkResultFunctor_ReturnsNewErrorResult()
    {
        var expected =
            new Functor<ResultType<DummyNewValue, DummyNewError>>(
                new ResultType<DummyNewValue, DummyNewError>.Error(NewError));
        (await Task.FromResult(ErrorResultFunctor).MatchAsync(OkMap, ErrorMap)).Should().BeEquivalentTo(expected);
    }
    ...
    public static DummyNewValue OkMap(DummyValue value) => new() { NameAllCaps = value.Name.ToUpper() };
    public static DummyNewError ErrorMap(DummyError error) => new() { Message = "new " + error.Message, Count = 2 };
    public static string StringErrorMap(string message) => "new " + message;
}