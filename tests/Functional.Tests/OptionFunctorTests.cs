...
public class OptionFunctorTests
{
    ...
    {
        var newOptionFunctor = SomeOptionFunctor.Map(value => new DummyNewValue { NameAllCaps = value.Name.ToUpper() });
    ...
    {
        var newOptionFunctor = NoneOptionFunctor.Map(value => new DummyNewValue { NameAllCaps = value.Name.ToUpper() });
    ...
    public async Task MapAsync_InputSomeOptionWithSynchronousLambda_ReturnNewSomeOption()
    {
        ...
        var newOptionFunctor =
            await optionFunctorTask.MapAsync(value => new DummyNewValue { NameAllCaps = value.Name.ToUpper() });
    ...
    public async Task MapAsync_InputSomeOptionWithAsyncLambda_ReturnNewSomeOption()
    {
        var optionFunctorTask = Task.FromResult(SomeOptionFunctor);
        var newOptionFunctor =
            await optionFunctorTask.MapAsync(value =>
                Task.FromResult(new DummyNewValue { NameAllCaps = value.Name.ToUpper() }));
