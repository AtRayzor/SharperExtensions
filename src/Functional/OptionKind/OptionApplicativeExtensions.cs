...
{
    public static IApplicative<OptionType<TNew>> DoMap<T, TNew>(IApplicative<OptionType<T>> applicative,
        Func<T, TNew> mapping)
    ...
    public static IApplicative<OptionType<TNew>> Apply<T, TNew>(this IApplicative<OptionType<T>> applicative,
    ...
    public static async Task<IApplicative<OptionType<TNew>>> ApplyAsync<T, TNew>(
        ...
        where T : notnull where TNew : notnull
        => (await application).Type switch
        {
            OptionType<Func<T, TNew>>.Some someMapping => await DoMapAsync(applicativeTask, someMapping.Value),
            _ => new Applicative<OptionType<TNew>>(new OptionType<TNew>.None())
        };
}