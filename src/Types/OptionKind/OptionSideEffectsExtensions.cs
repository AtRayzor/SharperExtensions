...
public static class OptionSideEffectsExtensions
{
    ...
    public static void ThrowIfNone<T, TException>(
        this ISideEffects<OptionType<T>> option,
        string message
    )
        where T : notnull
        where TException : Exception
    ...
    private static void ThrowExceptionIfIsNone<T, TException>(
        ...
        where TException : Exception
    {
        if (optionType is not OptionType<T>.None)
            return;
