...
public static partial class Option
{
    public static class Async
    {
        ...
        public static async Task<Option<T>> AwaitIfSome<T>(
            Option<Task<T>> optionTask
        )
            where T : notnull =>
...
public static class OptionAsyncExtensions
{
    ...
    public static ValueTask<T> ValueOrAsync<T>(
        ...
        where T : notnull
        => Option.Async.ValueOrAsync(optionValueTask, fallback);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Task<T> ValueOrAsync<T>(
        this Task<Option<T>> optionTask,
        T fallback
    )
        where T : notnull
        => Option.Async.ValueOrAsync(optionTask, fallback);
}