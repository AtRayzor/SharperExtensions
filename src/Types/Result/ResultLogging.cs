...
public static partial class Result
{
    ...
    {
        public static Result<T, TError> LogIfOk<T, TError>(Result<T, TError> result,
            Func<T, string> createMessage,
            ILogger logger,
            LogLevel logLevel = LogLevel.None) where T : notnull where TError : notnull
        {
        ...
        public static Result<T, TError> LogInformationIfOk<T, TError>(
            ...
            where T : notnull where TError : notnull => LogIfOk(result, createMessage, logger, LogLevel.Information);

        public static Result<T, TError> LogIfError<T, TError>(Result<T, TError> result,
            Func<TError, string> createMessage,
            ILogger logger, LogLevel logLevel = LogLevel.Error) where T : notnull where TError : notnull
        {
        ...
        public static Result<T, TError> LogIfError<T, TError>(Result<T, TError> result, EventId eventId,
            Func<TError, string> createMessage,
            ILogger logger, LogLevel logLevel = LogLevel.Error) where T : notnull where TError : notnull
        {
            if (result is Error<T, TError> error)
        ...
        public static Result<T, TError> LogIfError<T, TError>(
            ...
            where T : notnull where TError : IErrorWithMessage
        {
        ...
        public static Result<T, TError> LogIfError<T, TError>(
            ...
            where T : notnull where TError : ILoggableError
        {
            if (result is Error<T, TError> { Err: { Message: var errorMessage, LogLevel: var logLevel } })
            {
                logger.Log(logLevel, "{ }", errorMessage);
            }
...
public static class ResultLoggingExtensions
{
    public static Result<T, TError> LogIfOk<T, TError>(
        ...
        ILogger logger,
        LogLevel logLevel = LogLevel.None) where T : notnull where TError : notnull
        => Result.Logging.LogIfOk(result, createMessage, logger, logLevel);
    ...
    public static Result<T, TError> LogIfError<T, TError>(
        ...
        ILogger logger,
        LogLevel logLevel
    ) where T : notnull where TError : notnull, IErrorWithMessage =>
    ...
    public static Result<T, TError> LogIfError<T, TError>(
        ...
        where T : notnull where TError : ILoggableError
        => Result.Logging.LogIfError(result, logger);
}