...
public abstract record ResultType<T, TE>
...
public abstract record ResultType<T> : IImplementsFunctor, IImplementsMonad, IImplementsApplicative
    where T : notnull
{
    ...
    {
        internal override ResultType<T, string> BaseResultType =>
            new ResultType<T, string>.Ok(Value);
    }

    public record Error(string Message) : ResultType<T>
    {
        internal override ResultType<T, string> BaseResultType =>
            new ResultType<T, string>.Error(Message);
    }
}
