...
public static class ResultApplicativeExtensions
{
        ...
        where TNew : notnull
    {
        ...
        {
            ResultType<Func<T, TNew>, TE>.Ok mappingResult
                => DoMap(applicative, mappingResult.Value),
            ResultType<Func<T, TNew>, TE>.Error errorResult
                => new Applicative<ResultType<TNew, TE>>(
    ...
    public static async Task<IApplicative<ResultType<TNew, TE>>> ApplyAsync<T, TE, TNew>(
        ...
        where TNew : notnull
    {
        return (await applicationTask).Type switch
        {
            ResultType<Func<T, TNew>, TE>.Ok mappingResult
                => await DoMapAsync(applicativeTask, mappingResult.Value),
            ResultType<Func<T, TNew>, TE>.Error errorResult
                => new Applicative<ResultType<TNew, TE>>(
                    new ResultType<TNew, TE>.Error(errorResult.Err)
                )
