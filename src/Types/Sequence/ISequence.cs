...
public interface ISequence<T> : IEnumerable<T> where T : notnull
{
    ...
    /// </returns>
    Option.Option<T> GetValueOrNone(int index);
    ...
    /// the index is invalid.
    /// </returns>
    Option.Option<T> this[int index] { get; }
    ...
    /// </returns>
    Option.Option<int> IndexOf(T option);
    ...
    /// item, or None if the item is not found.
    /// </returns>
    Option.Option<int> LastIndexOf(T option);
